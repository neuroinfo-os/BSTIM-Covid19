...
def curves(model_i, start, n_weeks, county, save_plot=False):
   ...
   # print(res_train['y'].shape)
    ...
    print("DAS")
    print(map_vals.shape)
    print(type(counties))
    # print(counties['11005']['demographics'][('total', 2018)])
    ik= 0
    for key, _ in counties.items():
        n_people = counties[key]['demographics'][('total',2018)]
        map_vals[ik] = map_vals[ik] / n_people
        ik = ik+1
    ...
    if save_plot:
        plt.savefig("../figures/map_{}_{}_{}.pdf".format(county,start,n_weeks))
